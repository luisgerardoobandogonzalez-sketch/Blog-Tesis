...
export class BlogService {
  ...
  // --- NUEVO MÉTODO: Filtrar blogs por Tag ---
  getBlogsByTag(tag: string): Observable<Models.Blog.Blog[]> {
    const normalizedTag = tag.toLowerCase();
    const taggedBlogs = this.fakeBlogs.filter(b =>
      b.tags && b.tags.some(t => t.toLowerCase() === normalizedTag) && b.is_published
    );
    return of(taggedBlogs).pipe(delay(200));
  }
  ...
  createBlog(blogData: Partial<Models.Blog.Blog>, authorId: string): Observable<Models.Blog.Blog> {
    ...
    // GAMIFICATION: Award XP for creating a blog
    this.gamificationService.addXP(authorId, 'create_blog').subscribe();
  ...
  likeBlog(blogId: string): Observable<Models.Blog.Blog> {
    ...
    if (blog && !blog.currentUserHasLiked) {
      ...
      // GAMIFICATION: Award XP to author for receiving a like
      this.gamificationService.addXP(blog.author_id, 'receive_like').subscribe();
    }
    return of(blog!);
  }
  ...
  toggleFeaturedStatus(blogId: string): Observable<Models.Blog.Blog> {
    ...
    if (blog) {
      ...
      // GAMIFICATION: Award XP if featured
      if (blog.is_featured) {
        this.gamificationService.addXP(blog.author_id, 'blog_featured').subscribe();
      }
    }
    return of(blog!);
  }
